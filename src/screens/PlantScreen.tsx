  ...
  const handlePlant = async () => {
    ...
    try {
      ...
      await plantDrop({
        ...
        mode: mode === 'tourism' ? 0 : 1,
        priceLamports: BigInt(Math.round(priceSOL * 1e9)),
        expiryTs,
      });
      showToast('Drop planted! 🌱');
      setTimeout(() => nav.navigate('Explore'), 1200);
    } catch (e: unknown) {
      showToast(e instanceof Error ? e.message : 'Plant failed');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <View style={styles.root}>
        ...
        <MapView style={styles.map} region={region} onLongPress={handleLongPress}>
          {pin && (
            <Marker coordinate={{ latitude: pin.lat, longitude: pin.lng }} title="Drop location" pinColor="#00BFFF" />
          )}
        </MapView>
        <TouchableOpacity
          style={[styles.confirmBtn, !pin && styles.confirmBtnDisabled]}
          onPress={() => pin && setStep(2)}
          disabled={!pin}
        >
  ...
  return (
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
        ...
        <TextInput
          style={styles.input}
          placeholder="Drop name (max 32 chars)"
        ...
        <View style={styles.rarityRow}>
          {RARITIES.map(r => (
            <TouchableOpacity
              key={r}
              style={[styles.rarityCard, { borderColor: RARITY_CONFIG[r].color }, rarity === r && { backgroundColor: RARITY_CONFIG[r].color + '22' }]}
              onPress={() => setRarity(r)}
            >
        ...
        <TouchableOpacity
          style={[styles.plantBtn, (!name.trim() || !wallet.publicKey) && styles.plantBtnDisabled]}
          onPress={handlePlant}
